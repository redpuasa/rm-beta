        ...
        ?>
          ...
          <li class="nav-item dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
              ...
              if ($totalTicketsOverdue > 0) { ?>
                <div class="dropdown-divider"></div>
                <a href="<?php echo REQUESTS_PAGE . "?status=overdue"; ?>" class="dropdown-item">
      ...
      <div class="sidebar">
        ...
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            // $sectionPages1 = [REQUESTS_PAGE . "?search=" . $_GET['search']];
            ...
            if (isAdmin() || isServiceManager() || isServiceDeskManager() || isReviewer()) {
              $sectionPages = [SUMMARY_RESPONSE, SUMMARY_REQUESTOR, SUMMARY_PROJECTS, SUMMARY_OVERTIME, SUMMARY_MINISTRIES, SUMMARY_MONTH]; ?>
            ...
            ?>
              ...
              if (mysqli_num_rows($result) > 0) {
                while ($settingsRow = mysqli_fetch_assoc($result)) {
                  ...
                  if (!in_array($settingsRow["sectionName"], $sections)) {
                    $sections[] = $settingsRow["sectionName"];
                  }
                  // Add section icon into array if it doesn't exist
                  if (!in_array($settingsRow["sectionIcon"], $icons)) {
                    $icons[] = $settingsRow["sectionIcon"];
                  }
                  ...
                  <div class="col-sm-6">
                    <div class="form-group">
                      ...
                      <select name="requestorMinistryId" class="form-control select2" required>
                        ...
                        $query = "SELECT * FROM ministries ORDER BY name;";
                        $result = mysqli_query($mysqli, $query);
                  ...
                  <div class="col-sm-4">
                    <div class="form-group">
                      ...
                      <select name="serviceId" class="form-control <?php echo mysqli_num_rows($result) == 1 ? "select2Filled" : "select2"; ?>" required>
                        <?php
                        if (mysqli_num_rows($result) > 0) {
                          while ($servicesRow = mysqli_fetch_assoc($result)) {
                        ?>
                            <option value="<?php echo $servicesRow["id"]; ?>" <?php echo mysqli_num_rows($result) == 1 ? " selected" : ""; ?>><?php echo $servicesRow["name"]; ?></option>
                        <?php
                          }
                        }
                        ?>
                  ...
                  <div class="col-sm-4">
                    <div class="form-group">
                      ...
                      <select name="priorityId" class="form-control select2Filled" required>
                        ...
                        if (mysqli_num_rows($result) > 0) {
                          $num = 1;
                          while ($priorityRow = mysqli_fetch_assoc($result)) {
                        ?>
                            <option value="<?php echo $priorityRow["id"]; ?>" <?php echo $priorityRow["name"] == "Normal" ? " selected" : ""; ?>>
                              <?php echo $priorityRow["name"]; ?>
                              <!-- <?php if ($priorityRow["day"] > 1) { ?>
                      (<?php echo $priorityRow["day"] ?> days)
                      <?php } else { ?>
                      (24 hours) <?php } ?> -->
                            </option>
                        <?php
                  ...
                  <div class="col-sm-12">
                    <div class="form-group">
                      ...
                      <select name="projectId" class="form-control select2">
                        ...
                        if (mysqli_num_rows($result) > 0) {
                          $num = 1;
                          while ($projectRow = mysqli_fetch_assoc($result)) {
                        ?>
